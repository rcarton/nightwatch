...
CliRunner.prototype = {
  ...
  getTestSourceForSingle: function(targetPath) {
    ...
    if (!Utils.isScriptFile(testsource)) {
      if (fs.existsSync(testsource + '.js')) {
        testsource += '.js'
      } else if (fs.existsSync(testsource + '.ts')) {
        testsource += '.ts'
      }
    }
        ...
        child.on('result', function(childResult) {
          switch (childResult.type) {
            case 'testsuite_finished':
              ...
              globalResults.unstable.failed += childResult.unstable.failed;
              globalResults.unstable.passed += childResult.unstable.passed;
