  ...
  return this.currentTestSuite
    .on('testcase:finished', function(results, errors, time) {
      ...
      var tests = this.globalResults.modules[moduleKey].completed[this.currentTestSuite.currentTest] = {
        ...
        errors  : isTestcaseUnstable ? 0 : results.errors,
      ...
      if (isTestcaseUnstable) {
        // If testcase is marked unstable, increment failed or passed.
        // This is used for reporting no. of passed and failed tests with @unstable attribute.
        if (results.failed > 0 || results.errors > 0) {
          this.globalResults.unstable.failed++;
        } else {
          this.globalResults.unstable.passed++;
        }
      } else {
        // If testcase is NOT marked unstable, increment failed and errors.
    ...
    .then(function onTestSuiteResolved(testResults) {
      ...
      if (typeof process.send == 'function') {
        process.send(JSON.stringify({
          ...
          unstable: this.globalResults.unstable,
