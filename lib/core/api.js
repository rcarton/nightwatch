...
module.exports = new (function() {
  ...
  function loadAssertionFiles(dirPath, parent, abortOnFailure) {
    ...
    for (var i = 0, len = commandFiles.length; i < len; i++) {
      if (Utils.isScriptFile(commandFiles[i])) {
        var commandName = Utils.stripScriptExtension(path.basename(commandFiles[i]));
  ...
  function loadCommandFiles(context, parent, shouldLoadClientCommands) {
    ...
    relativePaths.forEach(function(relativePath) {
      ...
      for (var i = 0, len = commandFiles.length; i < len; i++) {
        commandName = Utils.stripScriptExtension(path.basename(commandFiles[i]));
        if (Utils.isScriptFile(commandFiles[i]) && commandName.substr(0, 1) !== '_') {
  ...
  function loadCustomCommands(dirPath, parent) {
    ...
    commandFiles.forEach(function(file) {
      ...
      } else if (Utils.isScriptFile(file)) {
        var commandModule = require(fullPath);
        var name = Utils.stripScriptExtension(path.basename(file));
  ...
  function loadPageObjects(dirPath, parent) {
    ...
    pageFiles.forEach(function(file) {
      ...
      if (fs.lstatSync(fullPath).isDirectory()) {
        ...
        var pageName = Utils.stripScriptExtension(path.basename(file));
